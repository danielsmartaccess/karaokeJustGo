    ...
    <div className="relative w-full aspect-video bg-[#07070f] border border-slate-700 rounded-xl overflow-hidden">
      ...
      {type === 'cta' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 bg-[#07070f]">
          ...
          <div
            className="text-white font-display font-black text-center leading-tight mt-1"
      ...
      {type === 'notice' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#07070f] border-2 border-amber-500/40">
          <div
            className="text-amber-400 font-display font-black text-center"
            style={{ fontSize: 'clamp(7px, 2vw, 13px)' }}
          >
            📣 {content?.title}
          </div>
      ...
      {type === 'qrcode' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#06000e] gap-2 p-3">
          <div className="bg-white p-2 rounded-lg">
            <QRCodeSVG value={participantUrl()} size={60} fgColor="#06000e" bgColor="#ffffff" />
          </div>
          <p className="text-green-400 text-[9px] font-mono text-center">
      ...
      {/* Type badge */}
      <div
        className={`absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-mono border ${meta.bg} ${meta.color}`}
      >
          ...
          {timeRemaining !== null && (
            <div className="flex items-center gap-1.5 mt-1.5">
              ...
              <span className="text-slate-400 font-mono text-sm">
                {fmtTime(timeRemaining)} restantes
              </span>
            </div>
          )}
        </div>
                ...
                >
                  ...
                  >
                    {isActive ? (
                      ...
                      <>
                        <Tv2 className="w-3.5 h-3.5" /> Exibir
                      </>
                    )}
                  </button>
                </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
            ...
            <div className="flex gap-3">
              <select
                value={ctaDuration ?? ''}
                onChange={(e) =>
                  setCtaDuration(e.target.value === '' ? null : Number(e.target.value))
                }
              ...
              <button
                ...
                onClick={() => {
                  sendCTA('📢', customCta.trim(), ctaDuration);
                  setCustomCta('');
                }}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
            ...
            <div className="flex gap-3">
              <select
                ...
                onChange={(e) =>
                  setNoticeDuration(e.target.value === '' ? null : Number(e.target.value))
                }
              ...
              <button
                ...
                onClick={() => {
                  sendNotice(customNotice.trim(), noticeDuration);
                  setCustomNotice('');
                }}
      ...
      <div className="flex items-center justify-between bg-slate-900 border border-slate-800 rounded-xl px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p
              className={`text-xs font-mono mt-0.5 ${isOnline ? 'text-green-400' : 'text-red-400'}`}
            >
        ...
        >
          <span
            className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all duration-300 ${isOnline ? 'left-6.5' : 'left-0.5'}`}
          />
        </button>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 space-y-2">
            <p className="text-slate-500 text-xs font-mono break-all">{participantUrl()}</p>
            <button
              onClick={() => {
                dispatch({
    ...
    <div>
      <div className="flex gap-0 mb-5 bg-slate-900 border border-slate-800 rounded-xl p-1">
        {(
          ...
          ] as { id: KaraokeSubTab; label: string; badge: number }[]
        ).map((t) => (
          <button
            key={t.id}
            onClick={() => setSub(t.id)}
          ...
          >
            ...
            {t.badge > 0 && (
              <span
                className={`text-xs px-1.5 py-0.5 rounded font-mono ${sub === t.id ? 'bg-purple-500/30 text-purple-300' : 'bg-slate-800 text-slate-600'}`}
              >
                {t.badge}
              </span>
            )}
          ...
          {currentPlaying ? (
            <div className="bg-slate-900 border border-green-500/25 rounded-2xl overflow-hidden">
              ...
              <div className="p-5 grid grid-cols-1 lg:grid-cols-2 gap-5">
                <div>
                  <p className="text-slate-600 text-xs font-mono uppercase tracking-wider mb-0.5">
                    Participante
                  </p>
                  ...
                  <h3 className="text-base font-display font-semibold text-purple-300">
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-slate-600 text-xs mt-0.5">
                        {entry.song.artist} · {entry.requestedAt}
                      </p>
                    </div>
          ...
          {queue.length === 0 && pendingQueue.length === 0 ? (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl">
              <EmptyState
                icon="📋"
                title="Fila vazia"
                description="Nenhum participante aguardando."
              />
            </div>
          ) : queue.length === 0 ? null : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-800 bg-slate-950/50">
                      ...
                      <th className="text-left py-3 px-4 text-slate-500 font-medium hidden md:table-cell">
                        Solicitação
                      </th>
                  ...
                  <tbody className="divide-y divide-slate-800/40">
                    {queue.map((entry, index) => (
                      <tr
                        key={entry.id}
                        className={`hover:bg-slate-800/25 transition-colors ${index === 0 ? 'border-l-2 border-l-cyan-500' : 'border-l-2 border-l-transparent'}`}
                      >
                        <td className="py-3 px-4">
                          <span
                            className={`font-display font-black text-lg ${index === 0 ? 'text-cyan-400' : 'text-slate-700'}`}
                          >
                            {index + 1}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => dispatch({ type: 'START_PLAYING', entryId: entry.id })}
                              disabled={!!currentPlaying}
                              title="Iniciar"
                            ...
                            <button
                              onClick={() => dispatch({ type: 'CANCEL_ENTRY', entryId: entry.id })}
                              title="Cancelar"
...
const CONNECTION_META: Record<ConnectionStatus, { label: string; cls: string }> = {
  live: { label: 'Ao vivo', cls: 'text-green-400' },
  connecting: { label: 'Conectando…', cls: 'text-amber-400' },
  demo: { label: 'Modo demo', cls: 'text-cyan-400' },
  error: { label: 'Sem conexão', cls: 'text-red-400' },
};

export default function HostView({ hostName, onExit }: HostViewProps) {
  const { connection } = useKaraoke();
      ...
      <header className="bg-slate-950 border-b border-purple-900/25 px-5 py-3.5 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-1.5 text-xs font-mono">
            <Activity className={`w-3 h-3 ${status.cls}`} />
            <span className={status.cls}>{status.label}</span>
          </div>
          <button
            onClick={onExit}
            title="Sair do painel do Host"
            className="flex items-center gap-2 rounded-lg px-1.5 py-1 hover:bg-slate-800/70 transition-colors"
          >
